...
require "../inc/config_perm_ajax.php";//一级目录和二级文件的访问权限判断
  if($ajax_onuserperm_editprem=='1'){
   ...
   if ($isdel=='1' && $ajax_onuserperm_deleprem=='1'){
   	$strSQL="update webmenu set dele='0' where id_webmenu='".$newsid."' and id_webmenu not in (1,2,3,4,5,6,7,8)";
	$objDB->Execute($strSQL);
   }else{
   
   if($newsid==1||$newsid==2||$newsid==3||$newsid==4||$newsid==5||$newsid==6||$newsid==7||$newsid==8){
