...
function newsdiredit(val1,val2,val3) {//val1＝点击的新闻id，val2判断是一级新闻目录还是二级新闻目录
	  if (val2=='2') {$.post('ajax_prenewsedit.php',{val1:val1,val3:val3},function(data) {//ajax_prenews1edit.php 处理回传
                             var myjson = '';eval('myjson=' + data + ';');
							 popprompt_editnewsdir2('请选择一级目录:', myjson.newsid_ajaxpreedit, '请编辑二级目录', myjson.name, myjson.selectid,'2',myjson.isdelperm,myjson.intro,'', function(newsdir2name,fatherid,newsid,isdel,intro) {
	  ...
	  if (val2=='1') {$.post('ajax_prenewsedit.php',{val1:val1,val3:val3},function(data) {//ajax_preedit.php 处理回传
                             var myjson = '';eval('myjson=' + data + ';');
							 popprompt_editnewsdir2('请编辑目录名称:', myjson.newsid_ajaxpreedit, '请编辑一级目录', myjson.name, myjson.selectid,'1',myjson.isdelperm,myjson.intro,'', function(newsdir2name,fatherid,newsid,isdel,intro) {
						        if (newsdir2name) {$.post('ajax_newsdirfinaledit.php',{newsdir2name:newsdir2name,fatherid:fatherid,newsid:newsid,isdel:isdel,intro:intro},function(data) {
                                   var myjson = '';eval('myjson=' + data + ';');
                                    popmessage(myjson.info, '友情提醒!',function() {window.location.reload();}); });
                                  };
		   ...
		   <?php  for($i=0;$i<sizeof($arrnewsdir1);$i++){?>
            <tr>
              <td height="30" align="left" valign="bottom" class="txt_leftmenu"><table width="100%" border="0" cellspacing="5" cellpadding="5">
                  <tr>
                    <td class="txt_lanmu">
					<?php if($onuserperm_editprem=='1'){echo '<span onclick=newsdiredit('.$arrnewsdir1[$i][id_proddir].',"1",'.$onuserperm_deleprem.'); style="cursor:pointer">'.$arrnewsdir1[$i][name].'</span>';} else{ echo $arrnewsdir1[$i][name];}?>
                    </td>
                  </tr>
                  <tr>
                    <td align="center"><table width="88%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
						...
						<?php
						$strSQL="SELECT name,id_proddir FROM productdir WHERE level='2' and dele='1' and fatherid='".$arrnewsdir1[$i][id_proddir]."' order by ordernum ASC";
                        $objDB->Execute($strSQL);
                        $arrnewsdir2=$objDB->GetRows();
						for($j=0;$j<sizeof($arrnewsdir2);$j++){if($onuserperm_editprem=='1'){
echo '<span onclick=newsdiredit('.$arrnewsdir2[$j][id_proddir].',"2",'.$onuserperm_deleprem.');  style="cursor:pointer">'.$arrnewsdir2[$j][name].'</span>'."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";}else{echo $arrnewsdir2[$j][name];}
						}
